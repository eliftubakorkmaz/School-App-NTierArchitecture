    ...
    IMapper mapper) : IStudentService
{
    ...
    {
        List<Student> students = studentRepository.GetAll().OrderBy(p => p.ClassRoomId).ThenBy(p => p.FirstName).ToList();
        return students;
    }

    public  async Task<PaginationResult<Student>>GetAllByClassRoomIdAsync(PaginationRequestDto request)
    {
        PaginationResult<Student> students = await studentRepository.GetAll().Where(p => p.ClassRoomId == request.Id).Where(search => search.FirstName.ToLower().Contains(request.Search.ToLower()) || search.LastName.ToLower().Contains(request.Search.ToLower()) || search.IdentityNumber.Contains(request.Search)).OrderBy(p => p.FirstName).ToPagedListAsync(request.PageNumber, request.PageSize);
        return students;
    }

    public string Update(UpdateStudentDto request)
    {
        UpdateStudentDtoValidator validationRules = new UpdateStudentDtoValidator();
